...
# ---- 2) Run YOLO to get masks ----
model = YOLO(MODEL_WEIGHTS)
res   = model.predict(IMG_PATH, save=False, augment=False)[0]
masks = res.masks.data.cpu().numpy()   # (N_instances, h_model, w_model)
...
for inst_idx, mask in enumerate(masks):
    ...
    mask_color = np.random.randint(0, 255, size=3, dtype=np.uint8) # Random color for each mask
    ...
    colors[is_boundary] = [1.0, 0.0, 0.0]  # red for boundary
...
# ---- 3) Visualize each instance’s full cloud with red boundaries ----
o3d.visualization.draw_geometries(pcs)

plane_normal = np.array([a, b, c])
plane_normal /= np.linalg.norm(plane_normal)
target_normal = np.array([0.0, 0.0, 1.0])  # Z-axis
...
# Step 3: Rotate the entire cloud
pcd.rotate(R)
...
ax.scatter(points[:, 0], points[:, 1], points[:, 2], s=1, c='blue')  # s=1 for point size
...
# full_pcd.points = o3d.utility.Vector3dVector(dxyz_all)
# full_pcd.colors = o3d.utility.Vector3dVector(rgb_all)

# # --- 1) Prepare your depth and intrinsics if not already ---
# H, W = depth.shape
# u_grid = np.tile(np.arange(W), (H,1))
# v_grid = np.tile(np.arange(H)[:,None], (1,W))

# # --- 2) For each mask, extract its 2D contour & back-project into 3D lines ---
# line_sets = []
# for idx, mask in enumerate(masks):
#     # a) Resize mask to original image size
#     m = cv2.resize(mask.astype(np.uint8), (W, H), interpolation=cv2.INTER_NEAREST)
#     # b) Find external contour (keep the largest one)
#     contours, _ = cv2.findContours(m, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
#     if not contours:
#         continue
#     contour = max(contours, key=lambda c: c.shape[0]).squeeze(1)  # (L,2)

#     # c) Back-project each (u,v) on contour to 3D
#     us, vs = contour[:,0], contour[:,1]
#     zs = depth[vs, us]
#     xs = (us - cx) * zs / fx
#     ys = (vs - cy) * zs / fy
#     pts3d = np.stack([xs, ys, zs], axis=1)

#     # d) Build a LineSet (connect in sequence + close loop)
#     L = pts3d.shape[0]
#     lines = [[i, i+1] for i in range(L-1)] + [[L-1, 0]]
...
# # --- 3) Visualize full cloud + all boundaries at once ---
# o3d.visualization.draw_geometries(
#     [full_pcd, *line_sets],
#     window_name="Full Point Cloud with Segmentation Boundaries",
...
# Visualize point cloud with bounding box
...
def calculate_area_from_random_points(points):
    ...
    for i in range(n):
        ...
        x2, y2 = ordered_points[(i + 1) % n]  # Wrap around to the first point
        area += (x1 * y2 - x2 * y1)
...
def get_section_bounds(mesh, z_min, z_max):
    ...
    #if you wnat visualis the section on a object
    #Convert Trimesh mesh and section to Open3D objects
    ''' ply to mesh'''
    ...
    # side=0
    # if abs(min_x)<198:
    #     side = max(abs(max_x) ,abs(min_x), abs(max_y) ,abs(min_y))
    #     #print(max_x,side, (2/3)*math.pi*(side**3))
    # area = math.pi*(side**3)*(2/3)
...
def auto_section_bounds(mesh, thickness):
    ...
    """
    # Compute the overall z-range of the mesh from its bounding box
    # overall_bounds = mesh.bounds  # shape: (2, 3) where row0 = min, row1 = max for x,y,z
    ''' ply to mesh'''
...
pts = np.asarray(downpcd.points)   # shape (N,3)
...
while z_hi <= z_max:
    ...
    if slab.shape[0] >= 3:
        ...
        area = polygon_area_xy(hull_pts)
        total_volume += area * dz

    z_lo += dz
    z_hi += dz

print(f"Estimated convex‐hull volume: {total_volume:.6f} m³")
